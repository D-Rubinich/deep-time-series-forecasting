'''
Recurrent Neural Network (RNN) backtest and hyperparameter optimization runner.
...
Usage:
    python src/backtest/run_rnn_backtest.py

References:
    - Documentation: https://unit8co.github.io/darts/generated_api/darts.models.forecasting.rnn_model.html

Notes: The RNN model in Darts has specific forecast restrictions:
    • It only supports output_chunk_length=1 and output_chunk_shift=0
    • If other values are configured, the model automatically adjusts these parameters
    and issues a warning
    • To handle multi-step forecasts, this pipeline automatically implements recursive
    forecasting for the required prediction_length steps, then discards the initial
...
config_dict = {
    ...
    'output_chunk_shift': 1,                # shift between training date and first prediction date
    'support_output_chunk_shift': False,    # If False, the model does not have native support for output_chunk_shift > 0
    
    #* Data parameters
    'data_format': 'multivariate',    # Defines the data structure: 'univariate' or 'multivariate'
    ...
    #* Optuna parameters 
    'run_optuna': True,
    'optuna_storage_path': None,            # None uses default path (data/optuna/optuna_study.db",
    ...
    'optuna_n_trials': None,                # Use either N_TRIALS OR TIMEOUT 
    'optuna_timeout': 60*60*3,              # Use either N_TRIALS OR TIMEOUT
    ...
    'tensorboard_path_optuna':None,         # None uses default path (logs/tensorboard_logs/optuna)
...
model_config = {
    ...
    'lr_scheduler_params' : {
        ...
        "lr_scheduler_kwargs": {
            "interval": "step",
            "frequency": 1,
            'total_steps': 12601,   # Updated by function calculate_rate_scheduler_total_steps inside optuna study
            "max_lr": 1e-4,
            "pct_start": 0.20,
            "div_factor": 40,
            "final_div_factor": 75,
...
param_search_space = {
    ...
    #* Model Architecture parameters
    'model': {'type': 'categorical', 'values': ["RNN", "LSTM", "GRU"]},
    'hidden_dim': {'type': 'categorical', 'values': [8, 16, 32, 64, 128, 256]},
    'n_rnn_layers': {'type': 'int', 'low': 1, 'high': 4},
    ...
    'weight_decay': {'type': 'float', 'low': 1e-5, 'high': 5e-2, 'log': True},
    
    #* Learning Rate Scheduler parameters
    'max_lr': { 'type': 'float', 'low': 1e-5, 'high': 1e-2, 'log': True},
...
##* 2. Multivariate format - Updates in Parameters Search space and Models Config
if config_dict['data_format'] == 'multivariate':
    ...
    param_search_space.update({
        ...
        # Model Architecture parameters
        'hidden_dim': {'type': 'categorical', 'values': [32, 64, 128, 256]},
