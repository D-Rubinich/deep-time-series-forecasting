'''
TSMixer (Time-Series Mixer) backtest and hyperparameter optimization runner.
...
Usage:
    python src/backtest/run_txmixer_backtest.py

References:
- Model Paper: https://arxiv.org/abs/2303.06053
...
config_dict = {
    ...
    'optuna_timeout': 60*60*2,                 # Use either N_TRIALS OR TIMEOUT
...
model_config = {
    ...
    'model_architecture_params' : {
        ...
        'num_blocks': 1,
        "dropout": 0.1,
        "use_reversible_instance_norm": True,       # initial studies indicated a much better performance with reversible instance norm
...
param_search_space = {
    ...
    #* Model Architecture parameters
    'hidden_size': {'type': 'categorical', 'values': [32, 64, 128, 256, 512]},
    'ff_size': {'type': 'categorical', 'values': [64, 128, 256, 512]},
    ...
    'weight_decay': {'type': 'float', 'low': 5e-5, 'high': 1e-3, 'log': True},
    
    #* Learning Rate Scheduler parameters
    'max_lr': { 'type': 'float', 'low': 1e-6, 'high': 1e-3, 'log': True},
    'pct_start': {'type': 'categorical', 'values': [0.20, 0.25, 0.30, 0.35]},
    'div_factor': {'type': 'categorical', 'values': [25, 30, 35, 40, 45, 50]},
    'final_div_factor': {'type': 'categorical', 'values': [50, 75, 100, 250, 500]},
