'''
TFT (Temporal Fusion Transformer) backtest and hyperparameter optimization runner.
...
Usage:
    python src/backtest/run_tft_backtest.py

References:
    - Model Paper: https://arxiv.org/abs/1912.09363
    - Documentation: https://unit8co.github.io/darts/generated_api/darts.models.forecasting.tft_model.html
'''
...
config_dict = {
    ...
    'optuna_timeout': 60*60*4,                 # Use either N_TRIALS OR TIMEOUT
...
param_search_space = {
    ...
    #* Model Architecture parameters
    'hidden_size': {'type': 'categorical', 'values': [64, 128, 256]},
    'lstm_layers': {'type': 'int', 'low': 1, 'high': 2},
    'num_attention_heads': {'type': 'int', 'low': 1, 'high': 3},
    'dropout': {'type': 'float', 'low': 0.05, 'high': 0.6},
    'use_reversible_instance_norm': {'type': 'categorical', 'values': [True, False]},
    
    #* Trainer parameters
    'batch_size': {'type': 'categorical', 'values': [128, 256, 512, 1024]},
    'gradient_clip_val': {'type': 'float', 'low': 0.3, 'high': 100.0, 'log': True},
    'weight_decay': {'type': 'float', 'low': 5e-5, 'high': 5e-2, 'log': True},
    ...
    'final_div_factor': {'type': 'categorical', 'values': [75, 100, 250, 500, 1000]},
...
if config_dict['data_format'] == 'multivariate':
    ...
    param_search_space.update({
        ...
        # Model Architecture parameters
        'hidden_size': {'type': 'categorical', 'values': [16, 32, 64, 128]},
        
        # Trainer parameters
        'batch_size': {'type': 'categorical', 'values': [4, 8, 16]},  #reduced due to less samples in the multivariate format
        
        # Learning Rate Scheduler parameters
        'pct_start': {'type': 'categorical', 'values': [0.10, 0.15, 0.20, 0.30]},
    })
